var methods={
  ...
  renderLight:function(){
    ...
    // for(i=0;i<8;i++){
    //   foo = i+1;
    //   methods.lightPawn+foo  new methods.pawn({type:'P',color:'white',currentX:foo,currentY:'2'});
    // }
  },
  renderDark:function(){
  ...
  queen:function(spec) {
    ...
    this.move = function(x,y){
      x = methods.alphaToNumeral(x);
      if(Math.abs(x - this.currentX) === Math.abs(y - this.currentY) || x === parseInt(this.currentX) || y === (this.currentY)){
        $('.c'+this.currentX+'.r'+this.currentY).html('');
        $('.c'+x+'.r'+y).html(spec.type);
        $('.c'+x+'.r'+y).css({'color':spec.color});
        this.currentX = x;
  ...
  rook:function(spec) {
    ...
    this.move = function(x,y){
      x = methods.alphaToNumeral(x);
      if(x === parseInt(this.currentX) || y === parseInt(this.currentY)){
        $('.c'+this.currentX+'.r'+this.currentY).html('');
        $('.c'+x+'.r'+y).html(spec.type);
        $('.c'+x+'.r'+y).css({'color':spec.color});
        this.currentX = x;
  ...
  pawn:function(spec) {
    ...
    this.move = function(x,y){
      x = methods.alphaToNumeral(x);
      if(x === parseInt(this.currentX) && y - this.currentY === 1){
        $('.c'+this.currentX+'.r'+this.currentY).html('');
        $('.c'+x+'.r'+y).html(spec.type);
        $('.c'+x+'.r'+y).css({'color':spec.color});
        this.currentX = x;
